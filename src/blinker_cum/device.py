...
from blinker_cum.httpclient import *
from blinker_cum.mqttclient import *
from blinker_cum.errors import *
...
class Device(object):
    ...
    async def main(self):
        ...
        # thread_scheduler_run = threading.Thread(target=self.scheduler_run, daemon=True)

        # start
        for task in tasks:
            task.start()
            time.sleep(1.5)
        # thread_scheduler_run.start()

        for task in tasks:
            task.join()
        # thread_scheduler_run.join()
